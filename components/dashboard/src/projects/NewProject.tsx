...
export default function NewProject() {
    ...
    const renderSelectRepository = () => {
        ...
        const renderRepos = () => (<div className="mt-8 w-96 border rounded-t-xl border-gray-100 flex-col">
            <div className="p-6 flex flex-col space-y-2">
                <ContextMenu classes="w-full left-0 cursor-pointer" menuEntries={getDropDownEntries(accounts)}>
                    <div className="w-full relative ">
                        <input className="w-full pr-8 cursor-pointer" readOnly type="text" value={selectedAccount || ""}></input>
            ...
            <div className="w-96 h-3 border rounded-b-xl border-gray-100 bg-gray-100"></div>
        </div>);

        const renderEmptyState = () => (<div className="mt-8 w-96 border rounded-xl border-gray-100 flex-col">
            <div className="p-10">
    ...
    const renderSelectTeam = () => (<>
        ...
        <div className="mt-8 w-96 border rounded-xl border-gray-100 flex-col">
            ...
            <div className="py-6 px-2 flex text-gray-500">
                <div className="w-full relative" onClick={() => setShowNewTeam(true)}>
                    <div className="space-x-2">New Team</div>
                    <img src={CaretDown} title="Select Account" className="filter-grayscale absolute top-1/2 right-3" />
                </div>
            </div>
            {(showNewTeam || !teams || teams.length === 0) && (
                <NewTeam onSuccess={(t) => setSelectedTeam(t)} />
            )}
        </div>
    </>);
...
function GitProviders(props: {
    ...
    return (
        <div className="mt-8 w-96 border rounded-t-xl border-gray-100 flex-col">
    ...
    return (
        <div className="w-full">
            <div className="w-full flex flex-row space-x-2 p-2">
                <input type="text" name="new-team-inline" value={teamName} placeholder="team-name" onChange={(e) => setTeamName(e.target.value)} />
                <button key={`new-team-inline-create`} className="ml-2 w-4/12" disabled={!teamName} onClick={() => onNewTeam()}>Create Team</button>
            </div>
